...
| 플릭스 온라인 미팅 | `flixx` | **백지 1영역** (미팅 스크립트 + FAQ 통합) | **120분** | 키워드 자동 (자체 완결형) |

> 주의: `exam-feedback.html`만 30분이고 온보딩/메타광고는 90분.
> `exam-flixx.html`은 세일즈처럼 **백지(blank) 자체 완결형** — 페이지 안에 `FLIXX_ITEMS`와 채점 로직이 들어있고 제출 시 `auto_score`를 계산해 저장한다. `exam-data-flixx.js`는 admin 동적 로드 404 방지용으로만 남아있는 stale 파일(응시 페이지는 사용 안 함). 문항 수가 적어서 의도된 차이로 보이지만, 일괄 변경 작업 시 빼먹기 쉬움.
